...
class DeepFakeClassifierDataset(Dataset):
    def __init__(self,
                 data_path="/home/yjheo/Deepfake/dataset/dfdc_facebook/train/",
                 fold=0,
    ...
    def __getitem__(self, index: int):
        ...
        while True:
            video, _, label, ori_video, _, _ = self.data[index]
            try:
                if self.mode == "train":
                    label = np.clip(label, self.label_smoothing, 1-self.label_smoothing)
                img_paths = list(Path(os.path.join(self.data_root, self.crops_dir, video)).rglob('*.png'))
                img_paths = sorted(img_paths, key=lambda img_path: int(str(img_path).split('/')[-1].split('_')[-2]))
                images = []
                for i in range(0, len(img_paths), 2):
                    ...
                    diff_path = os.path.join(self.data_root, "diffs", video, img_path[:-4] + "_diff.png")
                    try:
                    ...
                    if self.mode == "train" and self.hardcore and not self.rotation:
                        landmark_path = os.path.join(self.data_root, "landmarks", ori_video, img_path[:-4] + ".npy")
                        if os.path.exists(landmark_path) and random.random() < 0.8:  # 0.7
                ...
                return {"image":torch.cat(images), "labels": np.array((label,)), "video_name": video, "valid": valid_label, "rotations": rotation}
            except Exception as e:
                traceback.print_exc(file=sys.stdout)
                print("Broken image", os.path.join(self.data_root, self.crops_dir, video))
