...
def blackout_convex_hull(img,part_mask):
    try:
        rect = detector(img)[0]
...
def get_eyes(image, landmarks):
    ...
    try:
        res = cv2.resize(eye_image, dsize=(32, 32), interpolation=cv2.INTER_CUBIC)
    except Exception as e:
        res = cv2.resize(image, dsize=(32, 32), interpolation=cv2.INTER_CUBIC)
    return res

def remain_eyes(image, landmarks):
    image = image.copy()
    (x1, y1), (x2, y2) = landmarks[:2]
...
def remove_eyes(image, landmarks, part_mask):
    image = image.copy()
    (x1, y1), (x2, y2) = landmarks[:2]
    mask = np.zeros_like(image[..., 0])
    line = cv2.line(mask, (x1, y1), (x2, y2), color=(1), thickness=2)
...
def remove_nose(image, landmarks, part_mask):
    image = image.copy()
    (x1, y1), (x2, y2) = landmarks[:2]
    x3, y3 = landmarks[2]
    mask = np.zeros_like(image[..., 0])
...
def remove_mouth(image, landmarks, part_mask):
    image = image.copy()
    (x1, y1), (x2, y2) = landmarks[-2:]
    mask = np.zeros_like(image[..., 0])
    line = cv2.line(mask, (x1, y1), (x2, y2), color=(1), thickness=2)
...
def remove_landmark(image, landmarks, part_mask):
    if random.random() > 0.5:
        image, part_mask= remove_eyes(image, landmarks, part_mask)
    elif random.random() > 0.5:
        image, part_mask = remove_mouth(image, landmarks, part_mask)
    elif random.random() > 0.5:
        image, part_mask = remove_nose(image, landmarks, part_mask)
    ...
    def __getitem__(self, index: int):
        ...
        while True:
            ...
            try:
                ...
                #eyes_image = np.zeros([32,32,3],dtype=np.uint8) eye
                mask = np.zeros(image.shape[:2], dtype=np.uint8)
                part_mask = np.ones(image.shape, dtype=np.uint8)
                ...
                if self.mode == "train" and self.hardcore and not self.rotation:
                    ...
                    if os.path.exists(landmark_path): #0.7 and random.random() < 0.8
                        landmarks = np.load(landmark_path)
                        #eyes_image = get_eyes(image, landmarks) eye
                        #landmarks_dropout
                        #if random.random() < 0.5:
                        #    image = remain_eyes(image, landmarks)
                        if random.random() < 0.7:
                            image,part_mask = remove_landmark(image, landmarks, part_mask)
                    elif random.random() < 0.2: #0.2
                        blackout_convex_hull(image, part_mask)
                    elif random.random() < 0.1: #0.1
                        binary_mask = mask > 0.4 * 255
                        ...
                        while current_try < tries:
                            ...
                            if label < 0.5 or np.count_nonzero(mask * bitmap_msk) > 20:
                                mask *= bitmap_msk
                                image *= np.expand_dims(bitmap_msk, axis=-1)
                                part_mask *= np.expand_dims(bitmap_msk, axis=-1)
                                break
                            current_try += 1
                ...
                if self.transforms:
                    ...
                    #origin_data = self.transforms(image=origin_image)
                    #part_mask = self.transforms(image=part_mask)
                    #origin_image = origin_data["image"]
                    #part_mask = part_mask["image"]
                    image = data["image"]
                    mask = data["mask"]
                ...
                #predict_index = np.where(image == 0)

                #part_mask = np.where(image==0, 1, 0)
                #W, H, C = part_mask.shape

                #part_mask = img_to_tensor(part_mask)
                image = img_to_tensor(image, self.normalize)
                #eyes_image = img_to_tensor(eyes_image, self.normalize) #0524 eye
                #origin_image = img_to_tensor(origin_image, self.normalize)

                #part_face = origin_image[predict_index[2][:], predict_index[0][:], predict_index[1][:]]
                return {"image": image,
                        "labels": np.array((label,)), "img_name": os.path.join(video, img_file), "valid": valid_label, "rotations": rotation} # "origin_image": origin_image, "mask":part_mask,
            except Exception as e:
